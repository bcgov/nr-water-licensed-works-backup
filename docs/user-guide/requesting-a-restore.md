...
the features from the copy you choose, so every edit made since that copy
was taken is discarded along with whatever went wrong. Backups are taken
three times a week, so that can mean several days of real work.
...
3. **Decide, and say so explicitly.** Tell the maintainer you want a
...
If you want to see exactly what the maintainer runs, it's documented in the
